...
const HC_AUTH_KEY='happyCoding.community.auth.v1';
...
async function safetyClient(){
  if(!safetyClientPromise)safetyClientPromise=(async()=>{
    ...
    // Community auth moved to localStorage in v0.10. Use the exact same persisted
    // session key here so reports/moderation never fall back to stale tab-only auth.
    return createClient(HC_SUPABASE_URL,HC_PUBLIC_KEY,{auth:{storage:localStorage,storageKey:HC_AUTH_KEY,persistSession:true,autoRefreshToken:false,detectSessionInUrl:false}});
  })();
...
async function findPostFromCard(card){
  ...
  const author=(card.querySelector('header > span:first-child')?.textContent||'').split(' · ')[0].trim();
  ...
  if(body){const exact=data.filter(p=>p.body===body);return exact.length===1?exact[0]:null;}
...
async function reportCard(card){
  ...
  if(!post){window.toast?.('Não foi possível identificar este post com segurança. Atualize a comunidade e tente novamente.');return;}
...
async function moderatorState(){
  ...
  const {data,error}=await db.from('hc_admin_members').select('user_id').eq('user_id',session.user.id).limit(1);
...
document.addEventListener('happy:session-data-changed',()=>ensureModeratorButton());
