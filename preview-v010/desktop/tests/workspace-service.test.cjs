...
test('native workspace rejects remote/subframe callers and launches only dialog-selected Godot without shell',async t=>{
  ...
  let handler,currentWindow,selection=[],confirmation=0,spawnArgs;const opened=[];
  ...
  const service=mod.exports.createWorkspaceService({userData:dir,getTabs:()=>[],openUrl:url=>opened.push(url)});service.open();
  ...
  await handler(event,'save',{id,name:'Game',repository:'https://github.com/developer/game',links:['https://docs.godotengine.org/'],godotProject:'/etc/passwd'});
  assert.equal((await handler(event,'state')).data.projects[0].godotProject,'');
  const resumed=await handler(event,'resume',{id});assert.equal(resumed.ok,true);assert.deepEqual(resumed.data,{openedRepository:true,openedLinks:1,godotAvailable:false,godotStarted:false});
  assert.deepEqual(opened,['https://github.com/developer/game','https://docs.godotengine.org/']);
  assert.equal((await handler(event,'openIssue',{url:'https://evil.example/a/b/issues/1'})).ok,false);
  assert.equal((await handler(event,'openIssue',{url:'https://github.com/developer/game/issues/7?utm_source=x#fragment'})).ok,true);
  assert.equal(opened.at(-1),'https://github.com/developer/game/issues/7');
  ...
  spawnArgs=undefined;const resumedWithGodot=await handler(event,'resume',{id});assert.equal(resumedWithGodot.data.godotStarted,true);assert.ok(spawnArgs);
