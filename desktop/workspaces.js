...
function render(next){
  ...
  if(p){$('godotPath').textContent=p.godotProject||'Nenhum arquivo vinculado.';$('launchGodot').disabled=!p.godotProject||!state.godotConfigured;$('openRepository').disabled=!p.repository;$('openLinks').disabled=!p.links.length;}
...
async function save(){
  ...
  const next=await call('save',{id:selected,name:$('projectName').value,notes:$('notes').value,repository:repo,links});dirty=false;render(next);notice('Espaço salvo neste computador.');return true;
}
...
bind('removeProject',async()=>{if(!canLeave())return;const next=await call('remove',{id:selected});dirty=false;render(next);});
...
bind('githubDisconnect',async()=>{render(await call('disconnectGitHub'));notice('Conexão removida deste PC. Use Gerenciar permissões para revogá-la também no GitHub.');});
