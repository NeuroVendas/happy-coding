...
app.setPath('userData',dir);app.enableSandbox();
...
async function run(){
  ...
  win=new BaseWindow({width:1440,height:900,show:false});
  ...
  const panel=win.contentView.children.find(v=>v!==original);await once(panel.webContents,'did-finish-load');
  ...
  assert.equal(await views[0].webContents.executeJavaScript('innerWidth'),390);
  const audit=await invoke('audit');assert.ok(audit.issues.some(i=>i.kind==='acessibilidade'));assert.ok(audit.issues.some(i=>i.kind==='contraste'));
...
run().then(()=>finish(0)).catch(e=>{console.error(e);finish(1);});
